...
for id in "${IDS[@]}"; do
    ...
    if [[ ! "$id" =~ ^[0-9]+$ ]]; then
        echo "⚠️  Invalid session ID format: $id. Skipping."
        continue
    fi
    ...
    (
        ...
        if git apply --3way "../${id}.patch"; then
            ...
            if git diff --cached --quiet; then
                echo "ℹ️  No staged changes after applying patch. Skipping commit."
                exit 0
            fi
            ...
            gh pr create \
                ...
                --head "$BRANCH_NAME" \
                --base "$DEFAULT_BRANCH" || echo "⚠️ PR creation might have failed."
        else
            echo "❌ Failed to apply patch for $id. Skipping."
        fi
    )
